...
import { Filter, X, Star, Users, Briefcase, TrendingUp, BarChart2 } from 'lucide-react';
    ...
    setLocalFilters(prev => {
      ...
      if (category === 'rating') {
        ...
        if (array.includes(value)) {
          newFilters[category as 'experience' | 'contentType'] = array.filter(item => item !== value);
        } else {
          newFilters[category as 'experience' | 'contentType'] = [...array, value];
        }
      }
                ...
                {experienceLevels.map(level => (
                  ...
                  >
                    <input
                      ...
                      onChange={() => handleFilterChange('experience', level.id)}
                ...
                {[5, 4, 3, 2, 1].map((rating) => (
                  <button
                    ...
                    className={`flex items-center w-full p-2 rounded transition-colors ${
                      localFilters.rating === rating ? 'bg-navy-blue text-white' : 'hover:bg-gray-50'
                    }`}
                  >
                    <div className="flex">
                      {[...Array(rating)].map((_, i) => (
                        <Star
                          key={i}
                          size={16}
                          className={localFilters.rating === rating ? 'fill-current' : 'text-yellow-400 fill-current'}
                        />
                      ))}
                    </div>
              ...
              <div className="space-y-2">
                {sortOptions.map(option => (
                  <button
                    ...
                    className={`flex items-center w-full p-2 rounded transition-colors ${
                      localFilters.sortBy === option.id ? 'bg-navy-blue text-white' : 'hover:bg-gray-50'
                    }`}
                  >
